 ...
 */
class SGL_StringTest extends PHPUnit_Framework_TestCase {

    function testStripIniFileIllegalChars()
    {
        $target = 'these are legal chars';
        $targetLen = strlen($target);
        $this->assertEquals($targetLen, strlen(SGL_String::stripIniFileIllegalChars($target)));

        $target = 'contains illegal " character';
        $targetLen = strlen($target);
        $this->assertEquals($targetLen, strlen(SGL_String::stripIniFileIllegalChars($target)) +1);
    ...
    function testDirify()
    {
        ...
        foreach ($aControl as $k => $control) {
            ...
            $this->assertEquals($aExpected[$k], $ret);
        }
    }
    ...
    function testClean()
    {
        ...
        $this->assertEquals($clean, 'here is a string with tags');
        ...
        $this->assertEquals($cleanArray, $expectedArray);
        // more recursive
        $array  = array(
            'foo1'  => array(
    ...
    function test_removeCharacters()
    {
        ...
        $this->assertEquals(mb_detect_encoding($str), 'UTF-8');
        $ret = SGL_String::replaceAccents($str);
        $pattern = '/[^A-Z^a-z^0-9()\s]/';
        $this->assertNotRegExp($pattern, $ret);
        $this->assertEquals(mb_detect_encoding($ret), 'ASCII');
    }
